...
//	oss << p;
	...
	OptionalArgument<unsigned> sharedL2CacheSize(&args, "L2_size", "shared L2 size (KB)", 2048);
...
//	OptionalArgument<string> periodType(&args, "period_type", "type of the partition recalculation period (cycles|instructions)", "cycles");
	...
	OptionalArgument<string> migrationPolicy(&args, "migration_policy", "migration policy (no_migration|multi_queue|first_touch|double_clock|frequency|offline)", "multi_queue");
	...
	OptionalArgument<uint64> pcmMigrationCost(&args, "pcm_migration_cost", "PCM migration cost", 1200);
...
//	OptionalArgument<bool> fixedPcmMigrationCost(&args, "fixed_pcm_migration_cost", "whether the hybrid memory uses a fixed migration cost for page migrations from DRAM to PCM", false);
	OptionalArgument<uint64> dramMigrationCost(&args, "dram_migration_cost", "DRAM migration cost", 3000);
//	OptionalArgument<uint64> pcmMigrationCost(&args, "pcm_migration_cost", "PCM migration cost", 1200);
	...
	OptionalArgument<uint64> pcmOpenLatency(&args, "pcm_open_latency", "PCM open latency", 220); //4.4 slower array reads //old: 480); //120ns
	OptionalArgument<uint64> pcmCloseLatency(&args, "pcm_close_latency", "PCM close latency", 600); //12 slower array writes //old: 5600); //350ns for 128 bit write
	OptionalArgument<uint64> pcmAccessLatency(&args, "pcm_access_latency", "PCM access_latency", 50);
	...
	OptionalArgument<uint64> pcmBusLatency(&args, "pcm_bus_latency", "PCM bus latency", 40); //10ns @4GHz; 10ns == 4 transfer @ 400MHz (DDR-800)
